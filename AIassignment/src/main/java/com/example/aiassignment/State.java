        ...
        for (int i=0; i< N; i++)
        {
            for (int j = 0; j<N; j++)
            {
                if (parent.blocks[i][j].trim().isEmpty()) //search for the index of space in the state(where a tile can be moved)
                {
                    ...
                    {
                        String[][] a =Arrays.stream(parent.blocks).map(String[]::clone).toArray(String[][]::new);
                        //Arrays.stream(parent.blocks).map((String[] row) -> row.clone()).toArray((int length) -> new String[length][]);
                        a = swap(a,i,j,i-1,j);
